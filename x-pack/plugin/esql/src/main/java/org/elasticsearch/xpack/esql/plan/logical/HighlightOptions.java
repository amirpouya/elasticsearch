...
public record HighlightOptions(
    ...
    /**
     * Reads the {@code analyzer} option: a plain, nullable string naming a built-in or globally-registered analyzer.
     * The name is only resolved to an actual analyzer later ({@code Highlight#postAnalysisVerification} at analysis time
     * and {@code LocalExecutionPlanner#planHighlight} at execution), so here we only type-check that it is a string.
     */
